...
export default function MPosition3d (_parent, _content2d, _div) {
		...
		if(e.touches==undefined){
			...
			if(e.touches.length==1){
				...
				if(e.touches.length>=2){
					...
					if(self.distTach==0){
						self.distTach=self.getDistance(self.pT,self.pT1)
						sahDist=0;
						
						panStart.set(Math.round(self.pT.x*1), Math.round(self.pT.y*1));
						
					}else{
						nnnDist=self.getDistance(self.pT, self.pT1)
	...
	if (self.devas==false) {
		...
		_div.addEventListener('mousemove', self.stageMoveNew);
		
	} else {
		_div.addEventListener('touchstart', this.mouseDown, { passive: false, capture: true });
		_div.addEventListener('touchmove', self.stageMoveNew, { passive: false, capture: true });					
	}
