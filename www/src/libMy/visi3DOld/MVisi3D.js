...
export function MVisi3D (_contentHTML, _content2d, _devas, _directional, _efect, _event3DArr, _alpha) {
	...
	this.efect;
	if (_efect == true) {		
		/*this.efect = new MEffectArray(this,['RenderPass', 'OutlinePass', 'ShaderPass']);
		this.efect.init();
		this.efect.renderPass=true;
		this.efect.outlinePass=true;
		this.efect.shaderPass=true;//*/
	...
	this.render = function () {
		...
		if (this.efect) {
			trace("::::",this.efect,this.efect.render())
	...
	this.objShadow = function (obj, bol) {
		...
		if(obj.bs!=undefined)bbbbbb=false;
		if (bbbbbb == true) {
			obj.castShadow = bol;
...
MVisi3D.prototype = {
	...
	set arrOut (v) {
		this._arrOut = v;
		if(this.efect)this.efect.setValue('outline', null, this._arrOut);
		this.intRend = 1;
	},
	get arrOut () {
		return this._arrOut;
