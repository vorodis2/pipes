...
export function PM(visi3D, objbase, _param) { 	
    ...
    var lt=window.document.URL.toString();
    var al=lt.split("/")
    var linkk=''
...
export function  Hron (par, idObj, fun) {
    ...
    this.getLink=function(s){ 
        let rs=s;
        
        if(s.indexOf("http")!=-1){
            rs=this.object.res.src.replace("http://","https://")
        }else{
            let a=this.object.res.src.split("../")
            rs=this.par.link+a[a.length-1]
        }
    ...
    this.start=function(){ 
       ...
       if(this.object==null)this.object={}
        ...
        if(this.object.res!=undefined){
            ...
            let ss=this.object.res.src.replace("http://","https://")
            //mhbd.getLink(this.object.res.src)//.replace("http://","https://")

            var rr=this.getLink(this.object.res.src)
