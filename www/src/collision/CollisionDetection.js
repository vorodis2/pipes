...
function CollisionDetection() {
    ...
    this.correct = function (b) {
        ...
        if (this.activBox.rangeY != undefined && this.activBox.rangeY != null) {
            if (this.activBox.y < this.activBox.rangeY[0]) {
                this.activBox.y = this.activBox.rangeY[0];
            }
            if (this.activBox.y > this.activBox.rangeY[1]) {
                this.activBox.y = this.activBox.rangeY[1];
            } 
        } else if (this.activBox.rangeX != undefined && this.activBox.rangeX != null) {
            if (this.activBox.x < this.activBox.rangeX[0]) {
                this.activBox.x = this.activBox.rangeX[0];
            }
            if (this.activBox.x > this.activBox.rangeX[1]) {
                this.activBox.x = this.activBox.rangeX[1];
            } 
        }
        ...
        if (this.collisionBox.length == 0) { // пересечений нет
            ...
            if (!this.isInBigBox(this.activBox)) {
                if (this.activBox.funErr) this.activBox.funErr();
                isCorrect = false;
            }
            return isCorrect;
        } else {
    ...
    this.removePositFromHard = function(pos) {
        for (var i = 0; i < pos.length; i++) {
            if (this.activBox.rangeY != undefined && this.activBox.rangeY != null) {
                if (pos[i].y < this.activBox.rangeY[0]) {
                    pos.splice(i--,1);
                } else if (pos[i].y > this.activBox.rangeY[1]) {
                    pos.splice(i--,1);
                }
            } else if (this.activBox.rangeX != undefined && this.activBox.rangeX != null) {
                if (pos[i].x < this.activBox.rangeX[0]) {
                    pos.splice(i--,1);
                } else if (pos[i].x > this.activBox.rangeX[1]) {
                    pos.splice(i--,1);
                }
            }
        }
    ...
    this.updateWorld = function () {
        var len = this.arrBox.length;
        for (var i = 0; i < this.arrBox.length; i++) {
            this.activBox = this.arrBox[i];
            this.correct();
            if (len != this.arrBox.length) {// значит после коректировки были удалены блоки
                i = -1;// заново на проход
                len = this.arrBox.length;
            }
        }
    };
    ...
    this.checkBigBox = function () {
        ...
        for (var i = 0; i < this.arrBox.length; i++) {
            ...
            if (this.isInBigBox(this.arrBox[i])) {
                if (this.activBox && this.activBox.visiBox != undefined)
                    this.activBox.visiBox = true;
                ...
                if (this.isCollisionBigBox(this.arrBox[i])) {
                    ...
                    if (this.activBox && this.activBox == this.arrBox[i]) {
                        if (this.activBox.visiBox != undefined)
                            this.activBox.visiBox = false;
                    }
                }
    ...
    this.findAllBoxVect = function(p, curs, rect) {
        ...
        switch (curs) {
            case 1: { // верх лево
                return {x: arrX[0] ? 
                    arrX[0].x + arrX[0].width+(arrX[0].offset || 0)+(rect.offset || 0) : self.bigBox.x+(rect.offset || 0)
                    , y:  arrY[0] ? arrY[0].y + arrY[0].height+(arrY[0].offset || 0)+(rect.offset || 0) : self.bigBox.y + (rect.offset || 0)};
            }
            case 2: { // верх право
                return {x: arrX[0] ? 
                    arrX[0].x-(arrX[0].offset || 0)-(rect.offset || 0) : self.bigBox.x + self.bigBox.width-(rect.offset || 0)
                    , y: arrY[0] ? arrY[0].y + arrY[0].height+(arrY[0].offset || 0)+(rect.offset || 0) : self.bigBox.y +(rect.offset || 0)};
            }
            case 3: {// низ право
                return {x: arrX[0] ? 
                    arrX[0].x-(arrX[0].offset || 0)-(rect.offset || 0) : self.bigBox.x + self.bigBox.width-(rect.offset || 0)
                    , y: arrY[0] ? arrY[0].y-(arrY[0].offset || 0)-(rect.offset || 0)  : self.bigBox.y + self.bigBox.height - (rect.offset || 0)};
            }
            case 4: { // низ лево
                return {x: arrX[0] ? 
                    arrX[0].x + arrX[0].width+(arrX[0].offset || 0)+(rect.offset || 0) : self.bigBox.x+(rect.offset || 0)
