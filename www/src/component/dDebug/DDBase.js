...
export class DDBase{
    constructor(par,fun) {
        ...
        function setIndex (idArr){
            ...
            for (var i = 0; i < self.array.length; i++) {
                if(i==idArr){
                    ...
                    self.fun("addObject", self.array[i].object, self.array[i].tipRide,self.array[i].funDrag);
                    odin=self.array[i].object;
                    self.par.sizeWindow()
                    let ss="Нет информации в классе infoThis";
                    if(self.array[i].object)if(self.array[i].object.infoThis)ss=self.array[i].object.infoThis;
                    setTTT(ss)
                }else{
        ...
        this.sizeWindow = function(_w, _h, _s) {
            ...
            if(odin){
                if(odin.sizeWindow)odin.sizeWindow(www,hhh1,ssc)
            }
        } 
    }
