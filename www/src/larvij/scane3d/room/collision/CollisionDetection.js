...
export function CollisionDetection() {
	...
	this.findPosition2 = function (pos) {
		...
		for (var i = 0; i < this.collisionBox.length; i++) {
			...
			this.seeArrBox = [];
			this.findLevel = 0;
			postemp = this.freePosition(this.activBox, this.collisionBox[i], this.arrBox, true);
	...
	this.freePosition = function (box, boxCol, arrBox, bb) {
		this.findLevel++;
		var b=false
				...
				if (tempCol.length == 0) {  // колизий нету добавляем позицию					
					...
					if(bb!=undefined){
						...
						for (var j= 0; j< tempCol.length; j++) {
							if(tempCol[i])
							if(tempCol[i].idRandom!=box.idRandom){
								b=true
							}
						}
						if(b==false)freePos.push(arrPosit[i]);
	...
	this.getCollisionBox = function (box, arrBox, ofsetBox) {
		...
		//if(!box.active==false) return collisionBox;
		var b;
